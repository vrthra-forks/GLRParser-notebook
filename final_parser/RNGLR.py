...
class GSS:
    ...
    def create_node(self, state):
        print("**** CREATING NODE: ", self.label_count, "STATE:", state)
...
class SPPFNode:
    ...
    def check_exist_children_sequence(self, A):
        ...
        print("CHECKIGN TO MAKE PACKING", self)
        print(self.children)
        print(A)
...
class RNParseTableConstructer:
    ...
    def GOTO(self, state, sym):
        ...
        closure_rules = self.findClosure(copy.deepcopy(new_state))
    ...
    def findClosure(self, closure_rules):
        original_state = copy.deepcopy(closure_rules)
        ...
        if len(input_string) == 0:
            ...
            for u in self.U[n]:
                if u.state == self.accept_state:
                    ...
                    ### remove SPPF nodes that are not reachable from sppf_root


                    print(self.sppf_root)
                    self.result = True
        print(f"\n\n{self.result}\n\n")
        return self.result
    ...
    def reducer(self, i):
        ...
        for path_link in all_path:
            ...
            if m == 0:
                ...
                print((X), c, type((X)))
                for node in self.N:
                    print(node.label, node.start_position, type(node.label))
                    if node.label == (X) and node.start_position == c:
                        z = node
                        z_exists = True
                print("Z EXISTS", z_exists)
            ...
            print("PRINTINTG R IN REDUCER: ", self.R)
